  ...
  view(v) {
    ...
    return m(
      ...
      m(
        `button${cn}`,
        {
          onclick: () => (v.attrs.parentState.picIndex -= 1),
        },
        "prev"
      )
    );
...
const CloseButton = {
  oncreate: (v) =>
    window.addEventListener("resize", () => handleClose(v.attrs.parentVnode)),
  view(v) {
    return m(
      ".close-button",
      m(
        "button.flex",
        {
          onclick: () => handleClose(v.attrs.parentVnode),
        },
        m(
          ...
          m(
            "path[stroke=currentColor][fill=currentColor][d='m0 4.5 5-3m-5 3 5 3m45-3h-77']"
          )
        )
      )
    );
  ...
  return {
    ...
    oncreate(v) {
      ...
      root.addEventListener("swiped-left", (_) => {
        if (v.state.picIndex < v.state.picsArray.length - 1)
          v.state.picIndex += 1;
        m.redraw();
      });
      ...
      root.addEventListener("swiped-up", (_) => {
        cl('.close-button > button').click()
      });
    },
    view(v) {
      ...
      return m(
        ".modal.hidden[data-scroll][data-scroll-sticky][data-scroll-target=.main]",
        {
          style: {
              ...
              m(
                ...
                m(
                  ...
                  m(
                    ...
                    v.state.picsArray.map((url, i) =>
                      m(
                        `.thumb-wrap[data-index=${i}]${
                          i == v.state.picIndex ? ".selected" : ""
                        }`,
                        {
                          key: url,
                          onclick() {
