...
module.exports = {
  ...
  view(vnode) {
    ...
    return m('.footer[data-scroll].cormorant',
      m('.location.flex-column[data-scroll][data-scroll-speed=2]',
        m('.name', locales[lang].footer.me),
        m('.places', locales[lang].footer.places.map(p => m('.place', p)))
      ),
      m('.contacts.flex[data-scroll][data-scroll-speed=1]',
        m('.link-mail',
          m('.link-mail-text', locales[lang].footer.contact),
          m('a.lowercase.mail[href=mailto:kuslena@rambler.ru]', 'kuslena@inbox.ru')
        ),
        m('.socials',
          m('.socials-title', locales[lang].footer.alsoOn),
          m('.links.lowercase',
            m('a.insta-link.flex[rel=noopener][href=https://instagram.com/misskustova][target=_blank]',
              m('.link-text.insta-link-text', '@misskustova'),
              m(instaSvg),
            ),
            m('a.vk-link.flex[rel=noopener][href=https://vk.com/id17282149][target=_blank]',
      ...
      m(`.footer-nav.flex-column[data-scroll][data-scroll-speed=1][data-scroll-call=hide][data-scroll-repeat]`, {
        ...
        m('button.btt[data-scroll]', m('span', {
          onclick() { window.scroller.scrollTo("top") }
        }, locales[lang].footer.btt)),
        vnode.state.routes.filter(route => location.pathname != `/${route}`)
          .map(route => {
            return m(`a.${route}[data-scroll][href=/${route}]`, {
              ...
              m('span', locales[lang].footer[route])
            )
          })
      ),
      m('.credits[data-scroll][data-scroll-speed=2]',
        m('span', locales[lang].footer.creds), m('span', m('a.creds[rel=noopener]', {
